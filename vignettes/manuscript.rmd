...
where $\underline{\theta} = (\beta, \phi\, \sigma^2)$ and $p\left(Y_t = y_t | Y_{t-1}=y_{t-1}\right)$ is the conditional distribution of $y_t$ given $y_{t-1}$ and $p\left(Y_1=y_1 \right)$ is the distribution of the first point. For the model \@ref(eq:one) the log likelihood is given by

\begin{align}
logL\left( \underline{\theta}; \underline{y} \right ) = & -\frac{n}{2}log2\pi - nlog\sigma^2 + \frac{1}{2}log(1-\phi^2) \notag \\
& -\frac{1}{2\sigma^2}\left( (1-\phi^2)(y_1-\beta)^2 + \sum^n_{t=2}(y_t - \phi y_{t-1}-t\beta + \phi(t-1)\beta)^2 \right) (\#eq:three)
\end{align}
...
\begin{equation}
\hat\sigma^2 = \frac{1}{n}\left( (1-\phi^2)(y_1-\beta)^2 + \sum^n_{t=2}(y_t - \phi y_{t-1}-t\beta + \phi(t-1)\beta)^2 \right) (\#eq:four)
\end{equation}

Substituting $\hat\sigma^2$ into \@ref(eq:two) yields the log likelihood function as a function of $\beta$ and $\phi$


\begin{align}
logL\left( \beta, \phi; \underline{y} \right ) = &constant + \frac{1}{2}log(1-\phi^2) \notag \\
&-\frac{n}{2}log\left( (1-\phi^2)(y_1-\beta)^2 + \sum^n_{t=2}(y_t - \phi y_{t-1}-t\beta + \phi(t-1)\beta)^2 \right) (\#eq:five)
\end{align}

The maximum likelihood estimates for $\beta$ and $\phi$ are the values for which \@ref(eq:five) is maximized. This is akin to a Generalized Least Squares problem with an unknown covariance matrix, but with a known covariance structure. Maximization is achieved numerically and there are several ways to achieve this (@beach_maximum_1978;  @pinheiro_mixed_2000).
...
To assess the validity of the test we present some results from a larger simulation study. We used values of the trend, $\beta$ = 0, 0.12, 0.25, 0.5, the autoregressive parameter, $\phi$ = 0, 0.25, 0.5, 0.75, 0.95, the standard deviation of Gaussian process, $\sigma$ = 0.25, 0.5, 0.75 with time series of length n = 10, 25, 50, 100. We simulated 200 data sets for all combinations of the parameters above and 499 bootstrap samples. Nominal significance levels, testing at the 5% level, along with results using distributional theory are shown in Table \@ref(tab:table1). To ensure that the error time series is stationary, $\epsilon_1$ is drawn from  $N\left(0,\frac{\sigma^2}{(1-\phi^2)}\right)$.

When $H_0$ is true ($\beta = 0$), for the test to be valid, all entries in Table \@ref(tab:table1) should be close to 0.05 (±2 se, se=0.0154) when testing at the 5% significance level.  As can be seen from the results in Table \@ref(tab:table1), the significance levels from distributional theory are too liberal for small values of n. As $\phi \to 1$ the significance levels are too liberal for all values of n. However if n was large “enough”, the nominal levels would tend towards the true significance levels regardless of the level of $\phi$. In practice time series of the lengths required for this convergence are rarely observed. The bootstrap test presented does not suffer from this problem. The nominal levels are respectable for all sample sizes and all levels of autocorrelation, with the exception of very highly correlated data and small n (n = 10,  $\phi$ = 0.95). At this point the nominal levels are slightly high (~ 3 x standard error)
...
As an illustration, the test outlined above was applied to an annual riverflow discharge time series (Figure \@ref(fig:figure1)) from Cedar Rapids, IA (USGS station #05464500). The data is available from https://waterdata.usgs.gov. @hamed_modified_1998 used the same data set to apply their modified Mann-Kendall test for trend for which they conclude that there is insufficient evidence to reject the null hypothesis. We find a result to the contrary. The LR statistic $\Delta$ = 4.427 with a P-value = .046 using 1000 bootstrap samples. The Model fits under the null and alternative are shown in Figure \@ref(fig:figure1). Parameter estimates under the null are $\hat\alpha$ = 3560, $\hat\phi$ = .37, $\hat\sigma$ = 1670. Under the alternative $\hat\alpha$ = 2602, $\hat\beta$ = 20.88, $\hat\phi$ = .30, $\hat\sigma$ = 1630.

```{r figure1, eval = TRUE, echo =FALSE, out.width='70%', fig.align='center',fig.cap='Annual river flow discharge from Cedar Rapids, IA (USGS station \\#05464500). Dashed line shows model fit under $H_0$:$\\beta = 0$, dotted line shows model fit under $H_1$: $\\beta$ $\\neq 0$'}
...
df %>%
  kableExtra::kbl(.,col.names=c("$\\phi$",rep(c(" ","AR"),(ncol(df)-1)/2)),caption="Nominal levels under $H_0: \\beta = 0$. Autoregressive parameter, $\\phi$ = 0, 0.25, 0.5, 0.75, 0.95. Time series length n = 10, 25, 50, 100 and $\\sigma$ = 0.25, 0.5. The significance level from asymptotic result (AR) is also shown.") %>%
...
df2a %>%
  kableExtra::kbl(.,col.names=c(paste("$\\phi$"), rep(" ",(ncol(df2a)-1))),caption="Power of the test under $H_1: \\beta$ = 0.12, 0.25, 0.5. Autoregressive parameter, $\\phi$ = 0, 0.25, 0.5, 0.75, 0.95. Time series length n = 10, 25, 50 and $\\sigma$ = 0.25") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>%
...
df2b %>%
  kableExtra::kbl(.,col.names=c(paste("$\\phi$"), rep(" ",(ncol(df2b)-1))),caption="Power of the test under $H_1: \\beta$ = 0.12, 0.25, 0.5. Autoregressive parameter, $\\phi$ = 0, 0.25, 0.5, 0.75, 0.95. Time series length n = 10, 25, 50 and $\\sigma$ = 0.50") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kableExtra::add_header_above(c(" "= 1, "$\\beta$ = .12"=1, ".25"=1, ".50"=1, ".12"=1, ".25"=1, ".50"=1, ".12"=1, ".25"=1, ".50"=1)) %>%
  kableExtra::add_header_above(c(" "=1 ,"n = 10"=3,"25"=3,"50"=3)) %>%
  kableExtra::column_spec(1,border_right = T)
...
df2c %>%
  kableExtra::kbl(.,col.names=c(paste("$\\phi$"), rep(" ",(ncol(df2c)-1))),caption="Power of the test under $H_1: \\beta$ = 0.12, 0.25, 0.5. Autoregressive parameter, $\\phi$ = 0, 0.25, 0.5, 0.75, 0.95. Time series length n = 10, 25, 50 and $\\sigma$ = 0.75") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kableExtra::add_header_above(c(" "= 1, "$\\beta$ = .12"=1, ".25"=1, ".50"=1, ".12"=1, ".25"=1, ".50"=1, ".12"=1, ".25"=1, ".50"=1)) %>%
  kableExtra::add_header_above(c(" "=1 ,"n = 10"=3,"25"=3,"50"=3)) %>%
  kableExtra::column_spec(1,border_right = T)
...
df <- read.csv(here::here("output","table3.csv"),header=F)
df[is.na(df)] <- ""

df %>%
  kableExtra::kbl(.,col.names=c(paste("$\\phi_1$"), rep(" ",(ncol(df)-1))),caption="Nominal levels under $H_0: \\beta = 0$ with AR(2) errors. Autoregressive parameter, $\\phi_1$ = 0.1, 0.3, 0.5, 0.7 and $\\phi_2$ = 0.1, 0.2, 0.5, 0.7. Time series length n = 10, 25, 50, 100 and $\\sigma$ = 0.25.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kableExtra::add_header_above(c(" "= 1, "$\\phi_2$ = .1"=1, ".2"=1, ".5"=1, ".7"=1, ".1"=1,".2"=1, ".5"=1, ".7"=1, ".1"=1,".2"=1, ".5"=1, ".7"=1, ".1"=1,".2"=1, ".5"=1, ".7"=1)) %>%
  kableExtra::add_header_above(c(" "=1 ,"n = 10"=4,"25"=4,"50"=4,"100"=4)) %>%
  kableExtra::column_spec(1,border_right = T)
  #
