...
simulation_study_grid_ar2 <- function(outDir=here::here("out_ar2.txt"),
                                 betaVec = c(0),
                                 rho1Vec = c(0),
                                 rho2Vec = c(0),
                                 sigmaVec = c(0.25),
      ...
      for (nT in nTVec){
        for (sigma in sigmaVec) {
          ...
          for (isim in 1:nSims) {
            ...
            null <- fit_ar2_grid(data,rhoSearch,hypothesis="null")
            alt <- fit_ar2_grid(data,rhoSearch,hypothesis="alt")
            ...
            for (iboot in 2:nBootSims) {
              ...
              nullBoot <- fit_ar2_grid(bootdata,rhoSearch,hypothesis="null")
              altBoot <- fit_ar2_grid(bootdata,rhoSearch,hypothesis="alt")
              # statistic
